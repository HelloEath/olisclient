          ...
          <div>
            <div v-for="item in item9.value"  style="margin: 10px">
              ...
              <div style="font-size: 18px;margin-top: 10px;cursor: pointer">
                ...
                <router-link :to="{name:'mobileOlisDetails',query:{engine:engine,olis:JSON.stringify(item)}}" v-if="item.isClick==1">
                  <span style="color: red;cursor: pointer"> {{item.apiName}}</span></router-link>

                  <span style="color: red;cursor: pointer" v-else> {{item.apiName}}</span>
              </div>
